 ...
 <table class="table display table-bordered table-striped table-hover basic" id="howitwork">
  ...
  <?php foreach ($secTwoWork as $kye =>  $value) : ?>
    ...
    <tr>
      ...
      <td><?php echo $value->title; ?></php></td>
      <td><?php echo $value->description; ?></php></td>
      <td><?php echo $value->button_text; ?></php></td>
      <td>
          <form action="<?php echo base_url(route_to('delete-section-two-deatil',$value->id ))?>" id="sectiotwodetail" method="post" class="" accept-charset="utf-8" enctype="multipart/form-data">
                ...
                <button type="submit" class="btn btn-sm btn-danger" title= "<?php echo lang("Localize.delete") ?>"title= "<?php echo lang("Localize.delete") ?>"><i class="far fa-trash-alt"></i></button>
                <?php endif ?>
              </form>
      </td>
