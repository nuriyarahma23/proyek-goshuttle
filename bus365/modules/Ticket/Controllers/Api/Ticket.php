...
class Ticket extends BaseController
{
    ...
    public function busSeat($mainTripId,$journeyDate)
    {
       $bookSeat = array();

       $journeyDate =  date("Y-m-d", strtotime($journeyDate));
       $Seats =  $this->ticketModel->where('trip_id',$mainTripId)->where('journeydata',$journeyDate)->where('cancel_status',0)->where('refund',0)->findAll();

       foreach ($Seats as $key => $svalue) {
           
        array_push($bookSeat,$svalue->seatnumber);
       }
       $bookSeat = implode(",",$bookSeat);
       $bookSeat = explode(",",$bookSeat);
        ...
        $anotherarray = array();
        $lastSeat = null;
        $getTripDetails = $this->tripModel->find($mainTripId);
        $getFleetDetails = $this->fleetTypeModel->where('status',1)->find($getTripDetails->fleet_id);
        ...
        $seatloopslicenumber =  $totalseatnumber/$numberSeatRow;
