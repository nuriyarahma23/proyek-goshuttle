	...
	<div class="card mb-4">
		...
		<form action="<?php echo base_url(route_to('update-coupon', $coupon->id)) ?>" id="couponedit" method="post" class="row g-3" accept-charset="utf-8" enctype="multipart/form-data">
				...
				<div class="row">
					...
					<div class="col-4">
						...
						<label for="code" class=""><?php echo lang("Localize.coupon") ?> <?php echo lang("Localize.code") ?></label>
							<input type="text" id="code" name ="code" value="<?php echo esc(old('code') ?? $coupon->code ) ?>"  class="form-control text-capitalize"   required readonly>
						</div>
				...
				<div class="col-12 mt-3">
					...
					<label for="start_date" class="form-label"><?php echo lang("Localize.date") ?> </label>
					<div class="input-append date" id="start_date"  data-date-format="yyyy-mm-dd">
						<input size="16" type="text" name="start_date" class="form-control" value="<?php echo $coupon->start_date ?>"  required readonly>
				...
				<div class="col-12 mt-3">
						<label for="end_date" class="form-label"> <?php echo lang("Localize.date") ?></label>
						<div class="input-append date" id="end_date"  data-date-format="yyyy-mm-dd">
							<input size="16" type="text" name="end_date" class="form-control" value="<?php echo $coupon->end_date ?>" required readonly>
							<span class="add-on"><i class="icon-th"></i></span>
						</div>
				</div>
				...
				<label for="discount" class=""><?php echo lang("Localize.discount") ?> <?php echo lang("Localize.amount") ?></label>	
					<input type="text" id="discount" name ="discount" value="<?php echo esc(old('discount') ?? $coupon->discount ) ?>"  class="form-control text-capitalize"  placeholder="<?php echo lang("Localize.discount") ?> <?php echo lang("Localize.amount") ?>">
				</div>
