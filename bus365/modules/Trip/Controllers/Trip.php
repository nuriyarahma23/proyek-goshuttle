...
class Trip extends BaseController
{
...

	{
		...
		$stoppage = $this->request->getVar('stoppage');
		if (!empty($stoppage)) {
			$stoppage = implode(",", $stoppage);
		}
		...
		if($this->validation->run($validdata, 'trip'))
		{
			$this->db->transStart();
			
			$maintrip = $this->tripModel->insert($data);
			...
			$test =  $this->subtripModel->insert($subtripdata);
	...
	public function update($trip_id)
	{
		...
		else
		{
			...
			$driver = array();
			foreach ($this->stuffassignModel->where('trip_id ',$trip_id)->where('employee_type',1)->find() as $key => $driverValue) {
				array_push($driver,$driverValue->employee_id );
			}
			$assistant =  array();
			foreach ($this->stuffassignModel->where('trip_id ',$trip_id)->where('employee_type',2)->find() as $key => $assistantValue) {
				array_push($assistant,$assistantValue->employee_id );
			}
			...
			$data['trip'] = $this->tripModel->find($trip_id);
			$weekday = array(
				'1' =>'Monday',
				'2' =>'Tuesday',
				'3' =>'Wednesday',
			...
			$data['stand'] = $this->standModel->findAll();
			$data['facility'] = $this->facilitypModel->findAll();
			$data['arrival'] = $this->picdropModel->where('trip_id',$trip_id)->where('type',1)->findAll();
			$data['departure'] = $this->picdropModel->where('trip_id',$trip_id)->where('type',0)->findAll();
